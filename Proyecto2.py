...
def calcularJeerarquico(archivo, clusters):
    #Archivo=open('Datos1.csv','r')
    try:
        Archivo = open(archivo,'r')
        print(f'Jerarquico \nCon el archivo {archivo} \nk={clusters} ')
        ...
        jerarquico = AgglomerativeClustering(n_clusters=clusters).fit(datos)#500 es default
        #print(Clases)
        prediccion=jerarquico.labels_
        #print(kmeans.labels_)
        ...
        # print(Clases)
        metricas=precision_recall_fscore_support(Clases,prediccion)
        printMetricas(metricas,silhouette_score(datos,jerarquico.labels_))
